...
- 长篇、短篇、笔记三个分类分段。
- 每个分类分段展示对应分类的最新内容，卡片顶部标签分别为 `NOVELS`、`SHORTS` 和 `NOTES`；移动端保留分区标题与目录按钮在左、内容卡片在右的双栏关系。卡片标题固定为单行，超出可用宽度时在开头停顿一次；页面只渲染一份标题，标题向左完整滚出后经过间距从右侧重新进入。长篇卡片显示作品名，链接、日期和章序取最新章节，没有章节时回退到长篇作品；短篇和笔记卡片只在元信息中显示日期。
...
`/articles/` 只展示长篇、短篇、笔记三个分类入口。长篇、短篇、笔记列表页使用 `ArticleCard` 展示分类、标题、日期 / 分类 meta 和阅读入口。
