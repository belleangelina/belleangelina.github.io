...
当前顶部栏左侧还显示菱形标识和站名 `天然未来派的摸鱼小屋`。视口宽度不超过 `768px` 时隐藏站名文字，只保留菱形标识，避免长中文站名挤压导航。
...
- 默认使用浅色主题。
- 页面提供主题切换入口。
- 用户选择保存在浏览器本地的 `localStorage`，当前 key 为 `belle-theme-v2`。
- 只接受 `light` 和 `dark` 两个值。
- 主题切换完全在前端完成，不依赖后端。

## 14. 视觉风格

当前实现采用参考 `web/` 示例的渐变、玻璃感和轻动效视觉风格。
...
- 首页使用渐变主标题和打字机效果。
- 页面使用粉 / 紫 / 蓝体系的柔和强调色。
- 卡片和按钮使用轻玻璃感、细边框和克制 hover。
- 封面图可展示，但不喧宾夺主。
- 列表页清晰、安静，优先显示分类、标题、日期等信息，不展示摘要。
- 章节阅读页应尽量减少干扰，导航清楚但不过度突出。

整体气质偏“个人写作站 + web 示例视觉重写”，不是早期纯文学阅读站方向。

## 15. 移动端适配

V1 必须适配移动端。

移动端不是额外增强，而是基础阅读体验的一部分。实现时应采用响应式设计，保证手机、平板和桌面端都可正常阅读和导航。

规则：

- 使用移动优先或至少响应式布局。
- 页面主内容宽度在桌面端限制最大阅读宽度，移动端使用合理边距。
- 顶部导航在窄屏下不能拥挤；必要时改为简化导航或换行布局。
- 首页分类入口、文章列表、长篇目录在移动端应单列展示。
- 章节阅读页在移动端保持舒适字号、行距和段落间距。
- 上一章 / 返回目录 / 下一章导航在移动端应易点击，不应太小或过于拥挤。
- 暗色模式在移动端同样生效。
- 避免依赖 hover 才能使用的交互。
...
- 首页各分类分段的“最新”内容按 `date` 取最新项；长篇优先从章节中取最新项，没有章节时回退到长篇作品本身。
...
- 首页作为入口页：站名 Hero、打字机短文案、分类入口和各分类最新内容
- 文章总览页
- 长篇列表页
- 长篇作品页
- 卷页
...
当前主要设计项已实现。后续修改应以代码为准，并把已经落地的结构和行为同步回本文档。
