...
- 长篇、短篇、笔记三个分类分段。
- 每个分类分段展示对应分类的最新内容，卡片顶部标签分别为 `NOVELS`、`SHORTS` 和 `NOTES`；移动端保留分区标题与目录按钮在左、内容卡片在右的双栏关系。卡片标题固定为单行：未溢出的标题只渲染一份且不滚动；溢出的标题在开头停顿一次，再通过仅用于动画的副本，以 32px 间距单向衔接下一轮。长篇卡片显示作品名，链接、日期和章序取最新章节，没有章节时回退到长篇作品；短篇和笔记卡片只在元信息中显示日期。
